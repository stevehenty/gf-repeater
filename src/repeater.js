	...
	render() {
		...
		if ( items.length === 0 ) {
			...
			itemNodes = items.map((item, index) => {
				...
				return (
					<div className={ "gf-repeater-item " + itemName} data-id={item.id} key={item.id}>
							<div className="gf-handle">{handle}</div>
							<div className="gf-item-container">
								{el}
							</div>
						<div className="gf-buttons-container">
